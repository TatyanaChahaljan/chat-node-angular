...
let ChatComponent = class ChatComponent {
    constructor(chatService, dialog, fb) {
    ...
    ngOnInit() {
        this.showModal();
        this.subscriptions.push(this.chatService.userDisconnect().subscribe((socketData) => {
            if (!socketData) {
                return;
            }
            const { type, data } = socketData;
            this.connects.push({ type, name: `${data.name}` });
        }), this.chatService.getMessages().subscribe((socketData) => {
            const { data } = socketData;
            this.messages.push(data);
        }), this.chatService.getUser().subscribe((socketData) => {
            if (!socketData) {
                return;
            }
...
let PromptDialogComponent = class PromptDialogComponent {
